define([
  ...
  var ParticlesLayer = declare(null, {
    ...
    //创建OD线条
    createOdLines: function (context) {
      var startP = [];
      externalRenderers.toRenderCoordinates(this.view, this.startPoint, 0, this.view.spatialReference, startP, 0, 1);
      var position = this.jsonPoints;
      for (var k = 0; k < this.jsonPoints.length; k++) {
        var endP = [];
        externalRenderers.toRenderCoordinates(this.view, this.jsonPoints[k][0], 0, this.view.spatialReference, endP, 0, 1);
        //startParticles参数：起点位置，终点位置，抛物线高度，粒子运动速度
        this.particleSystems.startParticles(new THREE.Vector3(
          startP[0],
          startP[1],
          startP[2]
        ), new THREE.Vector3(
    ...
    createLabel: function () {
      var startP = [];
      externalRenderers.toRenderCoordinates(this.view, this.startPoint, 0, this.view.spatialReference, startP, 0, 1);
      let spriteMap = new THREE.TextureLoader().load('images/billboard.png');
      var spriteMaterial = new THREE.SpriteMaterial({ map: spriteMap });
      let sprite = new THREE.Sprite(spriteMaterial);
      sprite.position.set(startP[0], startP[1], startP[2] + 5000);
      sprite.scale.set(10000, 5000, 10000);
